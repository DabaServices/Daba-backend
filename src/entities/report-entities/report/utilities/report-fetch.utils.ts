...
const getToolCategory = (material?: Material) =>
  material?.standardGroupMaterials?.find(
    (standardGroupMaterial) =>
      standardGroupMaterial.standardGroup?.categoryGroup?.categoryDesc
        ?.description,
  )?.standardGroup?.categoryGroup?.categoryDesc?.description;
...
const buildMaterialDto = (
  ...
  nickname:
    material?.nickname?.nickname ?? standardGroup?.nickname?.nickname ?? '',
  ...
  for (const report of yesterdayInventoryReports ?? []) {
    for (const item of report.items ?? []) {
      ...
      const quantity = toNumber(
        item.confirmedQuantity ?? item.reportedQuantity,
      );
...
export const buildReportsResponse = ({
  ...
  if (
    !reports?.length &&
    !standaloneComments?.length &&
    !yesterdayInventoryReports?.length &&
    !screenAllocationReports?.length
  )
    return [];
  ...
  const yesterdayInventoryQuantityByUnitMaterial =
    buildYesterdayInventoryQuantityByUnitMaterial(yesterdayInventoryReports);
  ...
  for (const report of screenAllocationReports ?? []) {
    for (const item of report.items ?? []) {
      ...
      if (!materialById.has(item.materialId)) {
        materialById.set(
          item.materialId,
          buildMaterialDto(item.materialId, item.material, item.standardGroup),
        );
      }
  ...
  for (const report of reports ?? []) {
    ...
    const isScreenUnitReport =
      !isAllocationReport && report.unitId === screenUnitId;
    ...
    for (const item of reportItems) {
      ...
      if (!isAllocationReport && screenUnitCommentText) {
        addReportComment(
          reportCommentsByMaterial,
          item.materialId,
          report.reportTypeId,
          screenUnitCommentText,
        );
      }

      const incomingAllocationComment =
        isAllocationReport && report.recipientUnitId === screenUnitId
          ? resolveCommentByAuthor(
              item.material?.comments,
              report.unitId,
              report.recipientUnitId,
      ...
      if (incomingAllocationComment) {
        addReportComment(
          ...
          incomingAllocationComment,
        );
      }

      const childUnitComment =
        report.recipientUnitId === screenUnitId
          ? isAllocationReport
            ? ''
            : resolveCommentByAuthor(
      ...
      if (
        isAllocationReport ||
        (!isScreenUnitReport &&
          (toNumber(item.confirmedQuantity) !== 0 ||
            report.reportTypeId !== REPORT_TYPES.REQUEST))
      ) {
        itemByKey.set(key, {
          materialId: item.materialId,
          unitId: isAllocationReport
            ? (report.recipientUnitId ?? screenUnitId)
            : report.unitId,
          unit: isAllocationReport ? allocationRecipientUnit : reportingUnit,
          allocatedQuantity: isAllocationReport
            ? toNumber(item.confirmedQuantity)
            : null,
          type: {
            ...
            availableQuantityToEat: isAllocationReport
              ? toNumber(item.balanceQuantity)
              : 0,
            yesterdayInventoryQuantity:
              report.reportTypeId === REPORT_TYPES.INVENTORY
                ? (yesterdayInventoryQuantityByUnitMaterial.get(
                    `${report.unitId}:${item.materialId}`,
                  ) ?? null)
                : null,
    ...
    for (const item of report.items ?? []) {
      ...
      itemByKey.set(key, {
        ...
        type: {
          ...
          yesterdayInventoryQuantity: toNumber(
            item.confirmedQuantity ?? item.reportedQuantity,
          ),
          comment: '',
          status: item.status ?? null,
  ...
  for (const comment of standaloneComments ?? []) {
    ...
    if (!materialById.has(materialId)) {
      materialById.set(
        materialId,
        buildMaterialDto(materialId, comment.material, comment.standardGroup),
      );
    }

    if (
      reportTypeId === REPORT_TYPES.ALLOCATION &&
      commentRecipientUnitId === screenUnitId
    ) {
      addReportComment(
        reportCommentsByMaterial,
        materialId,
    ...
    const itemUnitAssociation = isAllocationComment
      ? comment.recipientUnit
      : comment.unit;
    const parentUnitAssociation = isAllocationComment
      ? comment.unit
      : comment.recipientUnit;
    const parentUnitId = isAllocationComment
      ? commentUnitId
      : (commentRecipientUnitId ?? screenUnitId);
  ...
  for (const aggregate of itemByKey.values()) {
    ...
    if (aggregate.allocatedQuantity !== null) {
      unitGroup.allocatedQuantity =
        (unitGroup.allocatedQuantity ?? 0) + aggregate.allocatedQuantity;
    }
  ...
  for (const materialId of materialIds) {
    ...
    const material =
      materialById.get(materialId) ?? buildMaterialDto(materialId);
    const comments = Array.from(
      reportCommentsByMaterial.get(materialId)?.entries() ?? [],
    )
    ...
    result.push({
      ...
      receivedAllocationQuantity:
        allocatedQuantityByMaterial.get(materialId) ?? null,
      quantityLeftToAllocate:
        quantityLeftToAllocateByMaterial.get(materialId) ?? null,
      items,
    });
  }
...
export const buildReportsMaterialsResponse = (
  params: FetchReportsParams,
): ReportDto[] => buildReportsResponse(params);
...
export const buildFavoriteReportsResponse = (
  ...
  childrenUnits: UnitRelation[],
  reportTypeIds: number[],
  yesterdayInventoryReports?: Report[] | null,
): FavoriteReportDto[] => {
